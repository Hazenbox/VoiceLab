 ...
 */
const SENSITIVE_EMOTIONS = ['raudra', 'bhayanak', 'karun', 'bibhatsa'];

/**
 * Intents that block nudges
 */
const SENSITIVE_INTENTS = ['complaint'];

/**
 * Resolution statuses that block non-critical nudges
 */
const BLOCKED_RESOLUTION_STATUSES = ['blocked', 'escalated'];

/**
 * Default nudge permission by user segment
 */
export const SEGMENT_PERMISSIONS: Record<string, NudgePermission> = {
  new: 'minimal',        // Don't overwhelm new users
  active: 'allowed',     // Standard nudges
  loyal: 'allowed',      // Standard nudges
  premium: 'ask_first',  // Respect premium users
  churning: 'never',     // Don't irritate potentially leaving users
};

/**
 * Relevance thresholds by permission level
 */
const RELEVANCE_THRESHOLDS: Record<NudgePermission, NudgeRelevance[]> = {
 ...
 */
export const NUDGE_LIBRARY: Record<string, Nudge[]> = {
  jio_telecom: [
    {
      type: 'expiry_warning',
      relevance: 'critical',

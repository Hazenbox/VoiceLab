...
const BRAND_PATTERNS: PatternRule[] = [
  // Demanding tone
  { id: 'ba-001', pattern: /\b(must|required|mandatory|compulsory)\b/gi, severity: 'warning', rule: 'Soften demanding tone', suggestion: 'Consider using softer language like "you can" or "we recommend"', category: 'tone' },
  ...
  { id: 'ba-003', pattern: /\b(not available|unavailable)\b/gi, severity: 'warning', rule: 'Frame positively', suggestion: 'Consider "coming soon" or "available in your area soon"', category: 'tone' },
  { id: 'ba-004', pattern: /\b(error|failed|failure)\b/gi, severity: 'info', rule: 'Use gentler language', suggestion: 'Consider "something went wrong" or "let\'s try again"', category: 'tone' },
  
  // Fear-based messaging
  { id: 'ba-005', pattern: /\b(urgent|hurry|last chance|only \d+ left)\b/gi, severity: 'warning', rule: 'Avoid fear-based urgency', suggestion: 'Use positive framing instead', category: 'fear' },
  { id: 'ba-006', pattern: /\b(act now|limited time|expires soon)\b/gi, severity: 'info', rule: 'Avoid pressure tactics', suggestion: 'Use specific date instead of urgency language', category: 'fear' },
  { id: 'ba-007', pattern: /\b(don\'t miss out|FOMO)\b/gi, severity: 'warning', rule: 'Avoid fear-based language', suggestion: 'Consider positive framing like "here for you when you need it"', category: 'fear' },
  
  // Boastful language
  { id: 'ba-008', pattern: /\b(best in India|number one|#1|most trusted)\b/gi, severity: 'warning', rule: 'Avoid boastful claims', suggestion: 'Focus on customer trust without superlatives', category: 'modesty' },
  ...
  { id: 'ba-012', pattern: /\b(pursuant to|in accordance with|notwithstanding)\b/gi, severity: 'warning', rule: 'Use everyday language', suggestion: 'Use simpler language like "following" or "based on"', category: 'jargon' },
  ...
  { id: 'ba-015', pattern: /\b(highly motivated|discerning customers|premium users)\b/gi, severity: 'warning', rule: 'Avoid exclusionary language', suggestion: 'Use inclusive language like "everyone" or "all users"', category: 'judgment' },
  
  // Marketing: Benefit-first
  { id: 'ba-016', pattern: /^(Diwali|Durga Puja|Holi|Eid|Christmas|New Year|Independence Day)\s+(sale|offer|special)\b/gim, severity: 'warning', rule: 'Headlines should lead with benefit, not event', suggestion: 'Put discount/benefit first in headline', category: 'marketing' },
  
  // Shame-inducing words
  { id: 'ba-017', pattern: /\b(you forgot|you missed|you failed to)\b/gi, severity: 'warning', rule: 'Avoid blame language', suggestion: 'Consider gentle reminders like "here\'s a reminder" or "still time to"', category: 'shame' },
  { id: 'ba-018', pattern: /\b(your fault|your mistake)\b/gi, severity: 'error', rule: 'Never blame the user', suggestion: 'Remove blame language and offer help instead', category: 'shame' },
